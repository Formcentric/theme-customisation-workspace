...
const fcThemesDir = path.join(__dirname, 'node_modules/@formcentric/client/dist/themes')
const themesDir = path.join(__dirname, 'src/themes')
const fcOutputFile = path.join(__dirname, 'src/util/fcThemesList.json')
const themesOutputFile = path.join(__dirname, 'src/util/themesList.json')
...
const generateThemesList = (dirPath, outputFile) => {
    ...
    fs.writeFileSync(outputFile, JSON.stringify(filteredFolders, null, 2))
}
